  ...
  (if-let [start (start-character? (in-fn))]
    (loop [msg {:start start
      ...
      (let [control (control-characters ch)]
        ...
        ;; Checksum
        (if (= :checksum control)
          (let [checksum (read-number in-fn)]
          ...
          (let [msg (update msg :checksum
            ...
            (cond
             ...
             ; :default msg ; FIXME: throw if unrecognized input

              )))))))

(def tst (str "$GPGGA,092750.000,5321.6802,N,00630.3372,W,1,8,1.03,61.7,M,55.2,M,,*76" \return \newline))

(let [in (java.io.ByteArrayInputStream. (.getBytes tst "US-ASCII"))
      unread (atom nil)]
  (read-message (fn
                  ([]
                   (if-let [ch @unread]
